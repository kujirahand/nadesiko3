...
このドキュメントはその続きで、**ファイル構成・データ形式・実装手順**を扱います。
...
#   合計: 180/231 通過 (77.9%)  不一致 51件 / 未実行 0件 / 非対応 3件
```
...
`cases/` と `expected/` は純粋なJSONなので、Go側には**同期スクリプトでコピー**し、
コピー元のコミットハッシュを `testdata/compat/SOURCE` に記録する運用を推奨します。
git submoduleでも構いませんが、なでしこ3リポジトリ全体を持ち込むことになるため、
`node_modules` を含まないコピーのほうが軽く済みます。
...
│   ├── gogen/             Goソース生成バックエンド（段階7）
│   └── compat/            差分fixtureの実行と結果出力
├── testdata/
│   └── compat/            本リポジトリからコピーした cases/ と expected/
└── docs/
```
...
| `要素数` / `LEN` / `ゼロ埋` / `空白埋` | コードポイント単位 | 同じ | ○ 一致（#2449 で対応） |
| ZWJ絵文字 `"👨‍👩‍👦"` の `文字数` | 5 | 5 | ○ 一致 |

### 一致しない2点

Goネイティブにすると差が出るのは、**次の2つだけ**です。
...
なお `要素数` / `LEN` / `ゼロ埋` / `空白埋` は、当初はUTF-16コードユニット基準のままで
`文字数` と食い違っていましたが、**#2449 でコードポイント基準に揃えられた**ため、
Go版との差はなくなりました。
...
- **ソース位置を必ず持つ。** エラー文面の行番号が互換対象なので落とせない
...
生成コードはVMが呼ぶのと同じ `internal/stdlib` をそのまま `import` します。
増えるのは変換器1つだけです。
...
| 段階 | 内容 | 完了条件 |
|---|---|---|
| 0 | fixtureのGo側同期、`value` / `text` / `ir` / `host` の定義、`compat run` の器 | `compat run` が全ケースを「未実装」として出力し、`compat:check` が動く |
...
| 4 | `event` と非同期命令 | `10` グループが通過。**全グループ通過（`11` の非対応3件を除く）** |
