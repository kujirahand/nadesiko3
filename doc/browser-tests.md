...
ブラウザ実行の入口は `test-browser/` に集約しています。
実行は `npm --prefix ./test-browser run ...` を使います。

- `npm --prefix ./test-browser run test:browser` … Browser プラグインの smoke test
- `npm --prefix ./test-browser run test:browser:full` … 旧来の browser 総合テスト
