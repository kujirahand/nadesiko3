...
なお、`pure` が `true` でない関数(`pure: false` または未定義。ただし asyncFn の非同期関数を除く。さらに「実行速度優先」で「強制ピュア」が指定されたブロック内ではエイリアス自体を行わない)がなでしこの関数内から呼び出された場合、呼び出しの間 `sys.__locals` は呼び出し元のローカル変数を保持する Map そのものを参照する(エイリアス)。`sys.__locals` への `set` / `delete` / `clear` は呼び出し元のローカル変数に直接作用するため注意すること(変数「それ」を含む)。`sys.__locals` 自体を別の Map に差し替えた場合は、呼び出し終了時にその呼出箇所より前で宣言済みのローカル変数名(「それ」を除く)のうち、差し替え後の Map にキーが存在するもののみが書き戻される。呼び出し終了後、`sys.__locals` は元の値に復元される。

なお、呼び出す命令の引数式は `sys.__locals` の差し替え前に評価される。そのため引数式の中で `sys.__locals` や `sys.__findVar` を参照する pure なプラグインは、差し替え前の `__locals` を観測する(呼出元のローカル変数は見えない。pure でないプラグインは引数位置でも自身のエイリアス期間を持つため影響を受けない)。
...
- sys.__findVar(name, def)
  - ローカル変数・名前空間変数の順に探し、値が falsy (`0`/`false`/`""`/`null` 等) であっても、最も内側で存在する変数の値を返す。ただし `undefined` は「未設定」として扱い、外側のスコープやデフォルト値 `def` へフォールバックする。
  - 関数の解決に使う場合は、結果が関数でない可能性があるため `sys.__findFunc(name, parentFunc)` (見つからない・関数でない場合はエラーを投げる) を使うこと。
- sys.__findFunc(nameStr, parentFunc)
  - 文字列なら変数名として関数を解決し、関数ならそのまま返す。文字列・関数以外の値や、解決結果が関数でない場合は `『parentFunc』に実行できない関数が指定されました。` というエラーを投げる。コールバック引数は登録時にこのメソッドで検証すること。
- sys.__exec(name, params)

他にも利用可能なメソッドやプロパティがあり、以下にまとめられている。

- [plugin_api.mts](../core/src/plugin_api.mts)
...
この機能の追加により、`「!非同期モード」`や`「逐次実行」`の利用は廃止となった。
