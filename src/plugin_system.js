...
const PluginSystem = {
  ...
  '文字抜き出': { /// 文字列SのA文字目からCNT文字を抽出する(v1非互換) /// もじぬきだす
    type: 'func',
    josi: [['で', 'の'], ['から'], ['を']],
    fn: function (s, a, cnt) {
      return (String(s).substr(a - 1, cnt))
  ...
  'カタカナか判定': { /// 文字列Sの1文字目がカタカナか判定 /// かたかなかはんてい
    type: 'func',
    josi: [['を', 'の', 'が']],
    fn: function (s) {
      const c = String(s).charCodeAt(0)
  ...
  '数字か判定': { /// 文字列Sの1文字目が数字か判定 /// すうじかはんてい
    type: 'func',
    josi: [['を', 'が']],
    fn: function (s) {
  ...
  '配列切り取': { /// 配列AのI番目(0起点)の要素を切り取って返す。Aの内容を書き換える。 /// はいれつきりとる
    type: 'func',
    josi: [['の'], ['を']],
    fn: function (a, i) {
      if (a instanceof Array) { // 配列ならOK
  ...
  '配列取り出': { /// 配列AのI番目(0起点)からCNT個の応訴を取り出して返す。Aの内容を書き換える /// はいれつとりだし
    type: 'func',
    josi: [['の'], ['から'], ['を']],
    fn: function (a, i, cnt) {
      if (a instanceof Array) { // 配列ならOK
