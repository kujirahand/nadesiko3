...
import { NakoCompiler, LoaderTool, newCompilerOptions } from '../core/src/nako3.mts'
import { NakoImportError } from '../core/src/nako_errors.mts'

import { Ast, CompilerOptions } from '../core/src/nako_types.mts'
import { NakoGlobal } from '../core/src/nako_global.mts'
import nakoVersion from '../src/nako_version.mts'

import { PluginDeno, fileExists } from './plugin_deno.ts'
import app from './commander_ja.ts'

import { NakoGenOptions } from '../core/src/nako_gen.mts'
...
export class CNako3 extends NakoCompiler {
  ...
  constructor (opts:CNako3Options = { nostd: false }) {
    ...
    if (!opts.nostd) {
      this.addPluginFile('PluginNode', posix.join(__dirname, 'plugin_node.mjs'), PluginDeno)
    }
    // 必要な定数を設定
