...
describe('calc_test.js', async () => {
  ...
  // 配列リテラル直後の添字アクセスのテストは array_test.mjs の『#1858』にまとめている
  it('JSON配列-リテラルへの添字アクセス', async () => {
    await cmp('[1,2,3][0]を表示', '1')
    await cmp('[[1,2],[3,4]][0][1]を表示', '2')
    await cmp('[1,2,3]@1を表示', '2')
  })
  ...
  it('bigint単項マイナスと増減文(#2488)', async () => {
    ...
    // 末尾 n のみ(空白を含む)の文字列は無効のまま(0n として黙って受け入れない) (#2488)
    await errorTest('A=1n;Aを「n」だけ増やす;Aを表示', 'NakoRuntimeError', 'Cannot convert n to a BigInt')
    await errorTest('A=1n;Aを「 n」だけ増やす;Aを表示', 'NakoRuntimeError', 'to a BigInt')
    await errorTest('A=1n;Aを「　n」だけ増やす;Aを表示', 'NakoRuntimeError', 'to a BigInt')
  ...
  it('助詞なし関数を代入の「は」の右辺で評価できる #2104', async () => {
    const defineMan = '●(A)万とは\nA×10000を戻す。\nここまで。\n'
    await cmp(`${defineMan}金額は20万。\n金額を表示。`, '200000')
    await cmp(`${defineMan}金額は(20万)。\n金額を表示。`, '200000')
  })
